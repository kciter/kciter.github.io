...
위와 같은 개념이 **펑터**라고 할 수 있다. 펑터는 타입을 확장하여 새로운 타입을 만들어내는 것이다. 이를 통해 에러를 담아낼 수 있다. 여기선 예제로 각각 대표적 약타입 언어인 JavaScript와 강타입 언어인 Kotlin을 사용했다.
...
위 코드는 Result라는 펑터를 구현한 코드이다. Result는 Sealed Class를 이용하여 Success와 Failure라는 서브 타입을 가지고 있다. Success는 성공적인 연산의 결과를 담고 있고 Failure는 에러를 담고 있다. 이제 위에서 구현한 divide 함수를 Result를 사용하여 다시 구현해보자.

# Railway-Oriented Programming이란?

간단하게 요약하자면 ROP는 사이드 이펙트를 제어하기 위한 함수형 패러다임 기반 방법론이다. 기본적으로 Result라는 Monad 객체를 구현하여 사용하고 에러를 체크하는 것은 어떤 방법을 사용하더라도 상관없다.
...
아쉽게도 몇 언어를 제외하면 Higher Kinded Type을 제공하는 언어는 드물기 때문에 Monad Comprehension을 사용할 수 없다는 문제가 있다.
