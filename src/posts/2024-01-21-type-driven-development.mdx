...
위 코드는 Idris 언어로 작성된 코드이다. 이해하기 힘든 코드지만 간단히 설명하자면 `Nat` 타입은 자연수를 의미하고 `Vect` 타입은 `Nat` 타입에 의존한다. 여기서 `Nat`은 `Vect`에서 길이를 나타내는 것으로 사용하고 있다. 따라서 다음과 같이 작성하면 재귀적으로 최대 길이 10인 `Vect` 타입을 만들 수 있다.
