...
<Image src="/images/2023-05-03-spring-multi-module-architecture/result.png" caption="소프트웨어 공학을 공부하면 자주 보는 그림" />
...
참고로 여기서 설명한 아키텍처는 모듈 레벨의 아키텍처 설계라 볼 수 있다. 조금 더 코드 레벨의 규칙은 따로 정해야한다. 그리고 현재 잘 관리하고 있지는 않지만 해당 아키텍처의 형태로 Spring 애플리케이션을 생성할 수 있는 [오픈 소스](https://github.com/cobaltinc/bloom)를 만들어두었다. 만약 이 아키텍처에 관심이 있다면 해당 프로젝트를 참고하면 좋을 것 같다.