...
JavaScript를 쓸 때 아쉬운 점은 IntelliSense[^1]를 사용할 수 없다는 점이다. 어느 정도 자동 완성을 해주긴 하지만 TypeScript의 강력함에 비하면 좀 아쉽다. 위 코드도 `as`를 통해 다른 요소를 사용하도록 변경했지만 어떤 속성을 넘길 수 있을지는 개발자가 잘 판단하여야 한다. 이런 문제점은 TypeScript에서 Polymorphic한 컴포넌트를 구현하면 해결할 수 있다.
...
여기까지는 이해하는 것이 어렵지는 않았을 것이다. 코드 양이 많은 것도 아니기 때문에 생각보다 쉽게 구현할 수 있다. 그렇지만 `ref`까지 사용하게 된다면 구현 난이도가 올라가게 된다. 일단 다음 코드를 살펴보자.
...
export const View = forwardRef(
  <T extends React.ElementType = "div">(
    { as, ...props }: ViewProps<T> & React.ComponentPropsWithoutRef<T>,
    // ref만 받아온다
