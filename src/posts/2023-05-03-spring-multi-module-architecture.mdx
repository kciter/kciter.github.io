...
그런데 만약 '내가 작성한 댓글 조회'라는 기능이 추가되고 어떤 게시물에 댓글을 남겼는지 정보가 필요하다고 가정해보자. 그러면 Article에 대한 정보가 필요하기 때문에 Comment 모듈에서 Article 모듈을 의존해야한다. 이런 경우 **순환 의존 관계**가 발생한다. 의존 관계에 순환이 발생하면 빌드가 되지 않는다.
