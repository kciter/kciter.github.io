...
런타임 환경에서 Kotlin Script를 실행할 수 있는 방법은 **Java Scripting API(JSR-223)**나 **Embeddable Host**를 사용하는 방법 두 가지가 있다. 그렇지만 사실 Java Scripting API가 내부적으론 Embeddable Host를 이용하기 때문에 같다고 볼 수 있다.
