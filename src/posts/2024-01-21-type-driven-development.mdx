...
앞서 단사 함수와 전사 함수 이야기를 한 것은 **타입을 통해 집합이 바뀌고 범위가 바뀔 수 있다는 것**을 말하고 싶었기 때문이다. 따라서 타입을 통해 로직을 추상화 하는 것이 가능하다. 프로그래머스의 [문자열 계산하기](https://school.programmers.co.kr/learn/courses/30/lessons/120902) 문제를 풀어본다고 가정해보자. 문제의 요구사항을 요약하면 다음과 같다.
