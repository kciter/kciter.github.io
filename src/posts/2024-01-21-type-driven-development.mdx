...
타입에 대해 깊게 생각해본 적이 없다면 단순히 언어에서 제공되는 원시 타입이나 클래스만을 생각할 수 있다. 하지만 타입은 그 이상으로 활용할 수 있는 개념이다. 타입은 함수와 데이터가 준수해야하는 **강력한 계약**이며 정적 타입 시스템은 이를 어길 수 없게 만든다. 개발자는 이 타입 시스템을 조금 더 풍부하게 사용하여 더 안정적인 프로그램을 만들거나 코드의 가독성을 좋게 만들 수 있다.
