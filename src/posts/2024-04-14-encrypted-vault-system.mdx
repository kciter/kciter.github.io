...
이제 본격적으로 비밀번호 관리자와 연관된 개인 비밀 금고에 대해 알아보자. 비밀번호 관리자는 기본적으로 두 가지 개념만 알아도 된다. 바로 **마스터 패스워드**와 **대칭 암호화**다. 실제 제품에는 보안을 위해 더 많은 기술이 필요하겠지만 가장 핵심적인 기술은 이 두 가지라고 할 수 있다.

## 마스터 패스워드

**마스터 패스워드는 비밀번화 관리자의 주인임을 인증할 수 있는 비밀 키**를 의미한다. 사용자는 마스터 패스워드를 통해 비밀번호 관리자 인증을 통과하고 비밀번호를 보관하거나 확인할 수 있다. 이말은 **마스터 패스워드이 유출되면 모든 비밀번호가 유출**된다는 것을 의미한다. 또한, 유실할 경우 저장된 모든 비밀번호를 알 수 없게 되버린다. 따라서 이 마스터 패스워드는 안전하게 보관해야 한다. 다만, 유실의 경우 다른 특수한 기술을 사용하여 복구하는 방법이 있을 수 있다.
...
마스터 패스워드를 비롯하여 안전한 비밀번호는 다음과 같은 특징을 가지고 있다. 만약 비밀번호 관리자를 사용하다면 가급적 다음 수칙을 지키는 것이 좋다.
...
def hash_master_password(master_password: str, salt: str) -> bytes:
    return hash_password(master_password, salt)
...
이제 이를 이용하여 새로운 비밀번호를 추가하고 저장된 비밀번호를 확인하고 삭제하는 모듈을 만들어보자.
...
참고로 이 글에서 쓰여진 코드는 정말 안전한지 검증된 것은 아니다. 그리고 만약 로컬이 아닌 네트워크 환경을 고려한다면 더 많은 것을 고려해야 한다. 따라서 이 글을 참고하여 실제 제품에 반영하고자 한다면 충분히 검증 후 도입하기를 권장한다. 여기서 사용된 알고리즘은 보편적으로 사용하는 것을 택했으며 더 안전한 방법이 있을 수 있다.
