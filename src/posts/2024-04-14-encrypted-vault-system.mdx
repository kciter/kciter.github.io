...
이제 이를 이용하여 비밀번호 관리자의 기본 기능을 구현해보자. 먼저 CLI 애플리케이션이기 때문에 `argparse`를 사용하여 명령어를 파싱할 것이다.
...
마스터 언록 키를 캐싱하는 등의 편의 기능은 여기서 구현하진 않았다. 만약 이 글의 내용이 재미있었다면 이를 추가로 구현해보는 것도 좋을 것이다. 이렇게 구현한 비밀번호 관리자는 로컬에서만 사용할 수 있으며 네트워크 환경에서 사용하는 경우에는 더 많은 보안을 고려해야 한다. 작성된 코드는 [GitHub 저장소](https://github.com/kciter/python-vault-example)에도 올려두었다.
