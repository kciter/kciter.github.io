...
const TRACKS = [
  { label: '테두리', start: 0, duration: 150, color: '#228be6', note: '150ms, ease-out' },
  { label: '크기', start: 0, duration: 500, color: '#fab005', note: '스프링' },
  { label: '상세', start: 50, duration: 400, color: '#fa5252', note: '400ms, ease-out' },
];
...
export const PropertySplitTimelineDemo = () => {
  ...
  useEffect(() => {
    ...
    const animate = (now: number) => {
      const elapsed = ((now - start) / 1000) * 1000;
      ...
      TRACKS.forEach((track, i) => {
        ...
        ctx.textAlign = 'left';
        ctx.textBaseline = 'middle';
        ctx.fillText(track.note, x0 + w + 6, y + TRACK_H / 2);
      });
