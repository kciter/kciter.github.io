...
타입에 대해 깊게 생각해본 적이 없다면 단순히 언어에서 제공되는 원시 타입이나 클래스만을 생각할 수 있다. 하지만 타입은 그 이상으로 활용할 수 있는 개념이다. 타입은 함수와 데이터가 준수해야하는 **강력한 계약**이며 정적 타입 시스템은 이를 어길 수 없게 만든다. 개발자는 이 타입 시스템 잘 활용하여 더 안정적인 프로그램을 만들거나 코드의 가독성을 좋게 만들 수 있다. 반대로 개발자가 타입을 풍부하게 사용하지 않는다면 타입 시스템을 통해 얻을 수 있는 이점을 제대로 누릴 수 없다.
...
마지막으로 정리하면 타입 시스템을 풍부하게 사용하는 것으로 다음과 같은 이점을 얻을 수 있다.
...
타입 시스템을 잘 이용하려면 개발자가 먼저 타입을 잘 이해해야 한다. 타입 시스템의 동작은 프로그래밍 언어마다 다르며 자신이 사용하는 언어에서는 지원하지 않더라도 다른 언어에선 지원할 수 있다. 따라서 타입 시스템을 잘 이해하고싶다면 다른 언어를 공부하는 것도 좋은 방법이다. 따라서 이 글에서는 다양한 타입 사용 사례를 소개하며 적절한 언어로 예제를 작성할 것이다.
