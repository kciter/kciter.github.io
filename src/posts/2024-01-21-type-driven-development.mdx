...
제네릭(Generic)[^1]은 대부분의 언어가 지원하므로 보편적으로 많이 사용하는 문법이다. 제네릭은 **포괄적인, 일반적인**이라는 뜻으로 프로그래밍 언어에서는 **다양한 타입을 포괄하는 타입**을 의미한다. 예를 들어 Kotlin에서 `List` 타입은 제네릭을 이용하여 `Int` 타입이나 `String` 타입을 모두 포함할 수 있다. 
