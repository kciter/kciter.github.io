...
타입을 잘 활용하기 위해서는 지식이 필요하다. 타입에 대한 여러 문법을 익히고 사용 사례와 패턴을 익힌다면 타입 시스템은 더이상 두려운 것이 아닌 든든한 친구가 될 수 있다. 이 글에서는 다양한 타입 사용 사례를 소개하면서 적절한 언어로 예제를 작성할 것이다. 다만, 타입 시스템의 동작은 프로그래밍 언어마다 다르기에 이 글에서 소개하는 문법이 모든 언어에서 지원되는 것은 아니다. 따라서 타입 시스템을 잘 이해하고싶다면 타입 시스템이 풍부한 다른 언어를 공부하는 것도 좋은 방법이다.

참고로 **타입을 과하게 이용하면 반대로 가독성이 안좋아질 수 있다**. 이는 코드의 문제가 아닌 협업 환경의 문제기 때문에 생소한 문법을 사용할 때는 내가 아는 것을 다른 팀원도 잘 알고있는지 파악하여 사용할 필요가 있다.

## 추상화

**타입 시스템을 이용한 추상화를 통해 중복 코드를 제거할 수 있다**. 보통 타입 시스템에서 가장 많이 활용하는 방법이라 볼 수 있다. 구조체, 클래스 등을 정의하는 것도 일종의 추상화라고 볼 수 있다. 이를 통해 코드의 가독성을 높일 수 있으며 중복 코드를 제거할 수 있다.

### 제네릭

타입을 통한 추상화 중 가장 많이 사용하는 방법 중 하나가 제네릭(Generic)[^1]일 것이다. 제네릭은 대부분의 언어가 지원하므로 보편적으로 많이 사용하는 문법이다. 제네릭은 **포괄적인, 일반적인**이라는 뜻으로 프로그래밍 언어에서는 **다양한 타입을 포괄하는 타입**을 의미한다. 예를 들어 Kotlin에서 `List` 타입은 제네릭을 이용하여 `Int` 타입이나 `String` 타입을 모두 포함할 수 있다. 
...
따라서 제네릭을 잘 사용한다면 코드 중복을 많이 줄일 수 있다. 여기까지는 대부분 잘 아는 내용이므로 좀 더 복잡한 내용을 살펴보자.

### 공변성과 반공변성

제네릭을 사용할 때 주의해야 할 점 중 하나가 **공변성(Covariance)**과 **반공변성(Contravariance)**이다. 이는 제네릭을 사용할 때 타입의 관계를 나타내는 것이다.

이를 이용하여 추상화를 더욱 풍부하게 할 수 있다. 예를 들어, `List` 타입은 `List<Int>` 타입을 포함하고 있지만 `List<Number>` 타입을 포함하고 있지 않다. 이는 `List<Int>` 타입은 `List<Number>` 타입의 하위 타입이 아니기 때문이다. 이를 해결하기 위해 공변성과 반공변성을 사용할 수 있다.
...
위 코드는 `List` 인터페이스를 정의한 코드이다. `List` 인터페이스는 `out` 키워드를 사용하여 공변성을 나타내고 있다. 이는 `List<Int>` 타입이 `List<Number>` 타입의 하위 타입이 될 수 있음을 의미한다. 반대로 `List` 인터페이스는 `in` 키워드를 사용하여 반공변성을 나타내고 있다. 이는 `List<Number>` 타입이 `List<Int>` 타입의 하위 타입이 될 수 있음을 의미한다.

이를 통해 코드의 중복을 줄이고 가독성을 높일 수 있다. 또한 타입 시스템을 통해 런타임 에러를 줄일 수 있다.

## 범위 조정하기

앞서 말했 듯이 타입 시스템이 제공하는 가장 큰 이점 중 하나는 **범위를 조정할 수 있다**는 것이다. 이는 앞서 설명한 것처럼 타입은 집합이며 함수를 통해 집합의 범위를 늘리거나 좁힐 수 있다.

범위를 조절하면 좋은 점으로 두 가지가 있다. 첫 번째는 **타입이 무엇을 의미하는지 명확하게 알 수 있다**는 것이다. 예를 들어 `String` 타입은 문자열을 나타내지만 그 범위가 너무 넓다. 반면 `Name`이라는 타입을 만든다면 이름을 나타낸다는 것을 타입만 봐도 바로 알 수 있다.

두 번째는 타입을 통해 **컴파일 타임에 오류를 검출할 수 있다**는 것이다. 예를 들어, 특정 함수가 Name 타입을 매개변수로 받도록 하면 잘못된 문자열이 들어가는 것을 방지할 수 있다. 이는 프로그램의 안정성을 크게 향상시킨다.

<h2>Algebraic Data Type</h2>

대수적 데이터 타입이라 부르는 ADT데이터 타입을 더 작은 데이터 타입의 조합으로 정의하는 방법이다. 대표적으로 합타입<sup>Sum Type</sup>과 곱타입<sup>Product Type</sup>이 있다.

<h2>Top Type, Bottom Type</h2>

최대 타입<sup>Top Type</sup>과 최소 타입<sup>Bottom Type</sup>을 이용하여 범위를 조정할 수 있다. 
...
타입 시스템의 다른 이점은 **행동을 제한할 수 있다**는 것이다.
...
타입을 먼저 정의한다는 점에서 테스트 주도 개발과도 잘 어울린다. 이미 잘 알려져있는 테스트 주도 개발은 내가 만들어야 할 기능의 테스트를 먼저 작성하고 이를 통과하는 코드를 작성하는 방법론이다.

입력과 출력 결과를 미리 정의하고 코드를 작성한다는 점에서 타입 주도 개발과 테스트 주도 개발은 비슷하다. 다만 타입만으로는 잡아낼 수 없는 오류나 예외 상황도 있기에 테스트 코드가 있다면 더욱 안전한 코드를 작성할 수 있다.
...
설계자의 입장에선 타입 시스템을 활용한다는 것은 코드 규칙을 만드는 것과 같다. 설계자는 이를 적절하게 이용하여 안전하면서 편리한 코드를 작성할 수 있게 간접적으로 코드 규칙을 만들 수 있다. 어떻게보면 간이 프레임워크를 만든다고 볼 수 있다.

[^1]: 참고로 외래어 표기법으로는 지네릭이라 표기한다.
[^2]: 다만 함수형 패러다임과 궁합이 좋다는 것은 부정할 수 없다.
