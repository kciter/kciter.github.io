---
series: "바퀴의 재발명 시리즈"
title: "음악 검색 시스템 만들기 like Shazam"
categories: article
...
위 코드에선 앞서 작성한 Rust 코드를 WASM으로 빌드하는 것과 [ffmpeg.wasm](https://github.com/ffmpegwasm/ffmpeg.wasm)을 이용하여 녹음된 오디오를 WAV로 변환하는 것을 어느정도 생략했다. 또한 미리 음원을 등록하는 부분도 생략했다.

핵심 코드만 살펴보면 녹음 버튼을 누르면 10초간 녹음한 후 녹음된 오디오를 WAV로 변환하여 검색하는 것이다. 검색 결과가 있으면 결과를 출력하고 없으면 "Not found"를 출력한다.

실제 테스트는 [링크](https://kciter.github.io/ringabell)에서 가능하다. 접속하면 기본적으로 5곡을 등록하며 추가로 곡을 등록하는 것도 가능하다. `Record and Search` 버튼을 클릭하면 녹음이 시작되며 10초간 녹음한 후 검색 결과를 확인할 수 있다.

# 마치며

이번 글에서는 음악 검색 시스템을 만들기 위한 알고리즘과 구현 방법에 대해 알아보았다. 앞서 생각보다 복잡하지 않다고 했지만 이는 앞서 치열하게 쌓아온 선배 개발자들의 노력 덕분이다. 다른 소리가 섞이거나 음악의 톤이 달라지는 경우, 리믹스 등의 경우에는 정확도가 떨어질 수 있지만 이를 잘 처리하는 것이 기술력이라고 볼 수 있다. 즉, 이 글에서 만든 음악 검색 시스템은 기본적인 기능만을 제공할 뿐이며 실제 서비스에 적용하기 위해서는 더 많은 연구와 개발이 필요하다.
