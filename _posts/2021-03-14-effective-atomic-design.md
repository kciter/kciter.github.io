...
<figure>
  ...
  <figcaption>Template은 와이어 프레임과 비슷한 개념이라 볼 수 있다</figcaption>
</figure>

Template은 흔히 사용해왔던 Layout과 비슷한 개념으로 볼 수도 있다. 더 정확하게 말하자면 실제 콘텐츠가 입혀지기 전 UI 요소, 레이아웃, 기능이 어떻게 배치될지 정하는 와이어 프레임이라고 볼 수 있다. 따라서 재사용이 자주 되는 컴포넌트가 된다.

### Page

<figure>
  <img src="/assets/img/2021-03-14-effective-atomic-design/page.png" />
  <figcaption>Page는 실제 데이터가 입혀진 사용자에게 보이는 화면 컴포넌트다</figcaption>
</figure>
...
네이버 메인을 내 기준대로 분석한다면 다음과 같다. 파란색이 Organism 주황색이 Molecule이다.
...
필자는 Side Effect가 발생하는 컴포넌트를 더러운 컴포넌트라고 부른다. 어감이 조금 강한 듯한 기분도 들지만 실제로 Side Effect 발생하는 로직이 포함되면 코드가 더러워지기 때문에 적합하다는 생각이 든다. 여하튼, 세상엔 때때로 때묻은 사람도 필요한 법이다. 마찬가지로 상황에 따라 컴포넌트 세상에도 더러운 컴포넌트가 필요하다.
...
```jsx
const Page = () => (
  // 물론 이것보다 훨씬 복잡하겠지만 대충 이런 모양이 아닐까?
