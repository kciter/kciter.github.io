...
타입을 잘 활용할 수록 안전하고 규칙을 통한 코드를 생산할 수 있다. 다만, 타입 시스템의 동작은 프로그래밍 언어마다 다르며 자신이 사용하는 언어에서는 지원하지 않더라도 다른 언어에선 지원할 수 있다. 따라서 타입 시스템을 잘 이해하고싶다면 다른 언어를 공부하는 것도 좋은 방법이다. 이 글에서는 다양한 타입 사용 사례를 소개하면서 적절한 언어로 예제를 작성할 것이다.

참고로 아이러니하게도 **타입을 과하게 이용하면 반대로 가독성이 안좋아질 수 있다**. 이는 코드의 문제가 아닌 협업 환경의 문제기 때문에 생소한 문법을 사용할 때는 내가 아는 것을 다른 팀원도 잘 알고있는지 파악하여 사용할 필요가 있다.
...
타입 시스템이 제공하는 가장 큰 이점 중 하나는 **범위를 조정할 수 있다**는 것이다. 이는 앞서 설명한 것처럼 타입은 집합이며 함수를 통해 집합의 범위를 늘리거나 좁힐 수 있다.
