...
이렇게 간단한 철학을 따르면서도 ROP는 사고적으로 굉장히 강력한 방법론이다. 우리는 프로그래밍을 할 때 항상 기능에 대한 추상화를 한다. ROP에선 **기능을 선로에 빗대어 추상화**하며 선로를 구성하는 기능들은 모두 성공 혹은 실패로 나뉜다. 이렇게 추상화를 하면 **기능의 단위를 적절한 성공과 실패로 나누게 되므로** 구현과 리팩토링 하는 것이 굉장히 쉬워진다.
