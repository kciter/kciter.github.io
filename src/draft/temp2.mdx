...
타입 시스템을 잘 활용하면 **추상화를 통해 중복 코드를 제거할 수 있다**. 보통 타입 시스템에서 가장 많이 활용하는 방법이라 볼 수 있다. 구조체, 클래스 등을 정의하는 것도 일종의 추상화라고 볼 수 있다. 이를 통해 코드의 가독성을 높일 수 있으며 중복 코드를 제거할 수 있다.
...
타입 시스템이 제공하는 가장 큰 이점 중 하나는 **범위를 제한할 수 있다**는 것이다. 이는 앞서 설명한 것처럼 타입은 집합이며 전사 함수를 통해 집합의 범위를 좁힐 수 있다.

범위를 좁히는 것의 이점은 두 가지가 있다. 첫 번째는 **타입이 무엇을 의미하는지 명확하게 알 수 있다**는 것이다. 예를 들어 `String` 타입은 문자열을 나타내지만 그 범위가 너무 넓다. 반면 `Name`이라는 타입을 만든다면 이름을 나타낸다는 것을 타입만 봐도 바로 알 수 있다.
